...
hold on 
ob1 = circle1(c1(1),c1(2),r1,'k');
hold on
ob2 = circle1(c2(1),c2(2),r2,'k');
hold on 
...
bf=0.1; bg=1.1;
r12=norm(p1-p2);
dt1=bf+bg*norm(u2min);
dt2=bg*norm(p2)*norm(u1max-u2min);
k1t=r12+(dt2/dt1);
dc=0.05;
...
%%
tspan = [0,Ts]';
x_in=[x1 y1 t1 x2 y2 t2 u1max' u2min']';
...
if ((t>800.1) && (t<=2200))
adv_cont12=a12*g1*Uc(1:2);
adv_cont13=a13*g1*Uc(1:2);
A1=[-0*a12*g1    a12*g2    zeros(1,2) -b12 0 0 zeros(1,3) zeros(1,3) zeros(1,3);
    -0*a13*g1  zeros(1,2)  a13*g3    0 -b13 0 zeros(1,3)  zeros(1,3) zeros(1,3);
    zeros(1,2) -a23*g2  a23*g3      0 0  -b23 zeros(1,3) zeros(1,3) zeros(1,3)];
b1= [b12+(adv_cont12) b13+(adv_cont13)  b23];
